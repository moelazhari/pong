...
const UserResult = ({ user }: { user: any }) => (
  <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-white/5 hover:bg-white/10 transition-all">
    ...
    <div className="flex-1 min-w-0">
      ...
      {user.status && <p className="text-xs text-gray-400 capitalize">{user.status.toLowerCase()}</p>}
    </div>
  </div>
)
  ...
  return (
    <div ref={divref} className="relative">
      <div className="relative">
        ...
        />
        <Search className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-gray-400" size={18} />
      </div>
